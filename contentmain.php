	...
	<div class = "flex-item">
		<!-- ITEM LOOP BEGIN -->
			<!-- QUARY PART START HERE -->
				<?php
				$q = 'SELECT listings.ItemID,Pic,Style,PIC2,PIC3,MemName,SetName FROM Listings LEFT JOIN PicComp on Listings.ItemID=PicComp.ItemID ORDER BY DateAdded DESC';
				$count=0;
				if ($res = $conn->query($q))
				{
